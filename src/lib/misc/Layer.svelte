<script lang="ts">
  ...
  const createRipple = (node: HTMLDivElement) => {
    ...
    const ripple = (e: MouseEvent) => {
      ...
      if (!navigator.userAgent.includes("Firefox") && size > 100) {
...
<div class="ripple-container" use:createRipple></div>
<div class="tint" class:tint-active={!useRipples}></div>
