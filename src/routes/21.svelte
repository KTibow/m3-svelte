<script lang="ts">
    ...
    let { showCode } = $props<{
        showCode: (name: string, minimalDemo: string, relevantLinks: {
            title: string; 
            link: string
        }[]) => void
    }>();
...
${"<"}/NavigationRail>`;
    const relevantLinks = [{"title":"NavigationRail.sv","link":"https://github.com/KTibow/m3-svelte/blob/main/src/lib/nav/NavigationRail.svelte"}];
</script>
...
<InternalCard title="Navigation Rail" showCode={() => showCode("Navigation Rail", minimalDemo, relevantLinks)}>
    ...
    {#snippet demo()}
        <div class="container">
            <NavigationRail fullyCollapse={fullyCollapse && !(!modal || !collapsible)} {collapsible} {modal}>
                {#snippet fab(open)}
